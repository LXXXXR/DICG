...
def plot_attn_stats(distance_vs_weight, exp_dir, penalty):
    ...
    for d in distance_vs_weight.keys():
        ...
        scatter_plt.scatter(
            [d] * len(weights), weights, color="red", s=0.1,
        )
        scatter_plt.scatter(d, distance_vs_avgweight[d], color="black", marker="_")
    scatter_plt.set_xlabel("distance")
    scatter_plt.set_ylabel("attention weight")

    distance_vs_avgweight = collections.OrderedDict(
        sorted(distance_vs_avgweight.items())
    )
    distance_vs_stdweight = collections.OrderedDict(
        sorted(distance_vs_stdweight.items())
    )
    distance_vs_n = collections.OrderedDict(sorted(distance_vs_n.items()))
    ...
    avg_weight_plt.fill_between(
        _x, _y - _y_err / 2, _y + _y_err / 2, color="blue", alpha=0.2
    )
...
def plot_attn_stats_simple(distance_vs_weight, exp_dir, penalty, n_samples, de):
    ...
    if de:
        ...
        tikzplotlib.save(
            figure=fig, filepath="./plots/penalty={}_DE_AttnWeights.tex".format(penalty)
        )
    else:
        fig.savefig("./plots/penalty={}_CE_AttnWeights.png".format(penalty))
        tikzplotlib.save(
            figure=fig, filepath="./plots/penalty={}_CE_AttnWeights.tex".format(penalty)
        )
...
def attn_analysis(unseeded_exp_dir, args, seeds, de=False):
    ...
    for seed in seeds:
        ...
        for i_eps in range(args.n_eval_episodes):
            print(
                "Seed = {}, Eval episode: {}/{}".format(
                    seed, i_eps + 1, args.n_eval_episodes
                )
            )
            obses = env.reset()
            ...
            for i_step in range(args.max_env_steps):
                ...
                actions, agent_infos = algo.policy.get_actions(
                    obses, env.get_avail_actions(), greedy=args.eval_greedy
                )
                if de:
                    attention_weights = algo.baseline.get_attention_weights(obses)
                    attention_weights_0 = attention_weights[0].detach().cpu().numpy()
                else:
