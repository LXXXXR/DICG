...
class GaussianMLPModule(nn.Module):
    ...
    def __init__(
        ...
        if share_std:
            ...
            if single_agent_action_dim is not None:
                init_std_param = torch.Tensor(
                    [init_std] * single_agent_action_dim
                ).log()
            else:
    ...
    def forward(self, inputs):
        ...
        if self._share_std:
            ...
            log_std_uncentered = (
                torch.zeros(*broadcast_shape, device=get_device()) + self._init_std
            )
        else:
        ...
        if self._share_std:
            ...
            if self._std_parameterization == "exp":
                ...
                std = torch.diag(log_std_uncentered.exp().exp().add(1.0).log())
