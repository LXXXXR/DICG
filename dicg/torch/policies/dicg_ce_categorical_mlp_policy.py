...
class DICGCECategoricalMLPPolicy(DICGBase):
    def __init__(
        ...
        # Policy layer
        self.categorical_output_layer = CategoricalMLPModule(
    ...
    def forward(self, obs_n, avail_actions_n):
        ...
        if not self.residual:
            dists_n = self.categorical_output_layer.forward(embeddings_collection[-1])
        else:
    ...
    def get_actions(self, obs_n, avail_actions_n, greedy=False):
        ...
        with torch.no_grad():
            ...
            agent_infos_n["action_probs"] = [
                dists_n.probs[i].cpu().numpy() for i in range(len(actions_n))
            ]
