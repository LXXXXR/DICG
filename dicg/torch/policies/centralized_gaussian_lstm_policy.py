...
class CentralizedGaussianLSTMPolicy(nn.Module):
    ...
    def forward(self, obs_n, avail_actions_n=None, actions_n=None):
        ...
        if self.state_include_actions:
            ...
            zero_pad = torch.zeros(
                n_paths, 1, self._n_agents, self._single_agent_action_dim
            )
        ...
        mean = mean.reshape(
            max_path_len, n_paths, self._n_agents, self._single_agent_action_dim
        ).transpose(0, 1)
        if self.share_std:
            std = std.reshape(
                max_path_len, n_paths, self._n_agents, self._single_agent_action_dim
    ...
    def step_forward(self, obs_n, avail_actions_n=None):
        ...
        if self.state_include_actions:
            ...
            if self._prev_actions is None:
                self._prev_actions = torch.zeros(
                    n_envs, self._n_agents, self._single_agent_action_dim
                )
        ...
        mean = mean.squeeze(0).reshape(
            n_envs, self._n_agents, self._single_agent_action_dim
        )
        if self.share_std:
            std = std.squeeze(0).reshape(
                n_envs, self._n_agents, self._single_agent_action_dim
            )
            dists_n = Independent(Normal(mean, std), 1)
        else:
